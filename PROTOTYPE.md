...
- Status pills: 🟢 مسدد (green) / 🔵 حساب قائم (blue) / 🔴 تجاوز الموعد (red) / ⚫ بلغ الحد (dark). "Due soon" (amber) styling exists (`st-amber`) but no customer currently uses it.
...
3. **New purchase (m-new):** select customer via chips **or** scan permanent profile QR (`openScanner('profile')` → simulate → selects Ahmed); live projected-balance line (`renderProj`) flags limit breach; optional description; **optional invoice upload** (see §Invoice). An overdue customer is flagged on their chip (red dot) before selection, and selecting one locks the submit button behind an explicit override checkbox (`setOverride`). Submit routes: over limit → **m-limit block screen** (with shortcut to edit that customer's limit); else `proceedSend`, which lands straight on the scanner.
4. **Two-way QR approval:** `proceedSend` creates `S.pending` + customer notification → the merchant goes straight to m-scan with the request summary on it (m-wait is still reachable from there for an absent customer). Switch to عميل → notification (actionable, highlighted) → c-purchase screen: amount/desc/after-balance (+invoice link if attached), canvas QR (deterministic from token via mulberry32), **real 2:00 countdown** (`startCountdown`, a green→amber→red progress bar plus the pill, pulses red ≤20s, expires → overlay + `regen()` button issues new token). Switch back to تاجر → m-scan → `simulateScan()` guards: no pending / customer hasn't opened QR (`shown`) / expired — each toasts; success applies balance, unshifts tx, recomputes due date (`dueFromDays`), marks the notif done, → m-success summary.
5. **Connect new customer:** m-add (mobile prefilled `0561 234 567`) → same QR dance → creates **فيصل الدوسري** with inherited defaults.
6. **Customer side:** dashboard stats + merchant cards; account detail with the deep-green primary "سداد الآن"; the permanent بطاقة التعرف (c-qr, ID-only, drawn lazily once); profile/logout.
...
15. **Settings:** merchant defaults; per-customer overrides (m-cust-settings) with checkbox-gated inputs showing "موروث من الافتراضي (…)" vs "قيمة مخصصة", save/reset; **demo tool** `demoOverdue()` flips Ahmed overdue (due 15 أغسطس, 3 days) to demo the locked path.
16. **Counter ergonomics:** any QR — the store code, بطاقة التعرف, either approval code — opens full screen on tap (`openQRFS`, nearest-neighbour upscale of the source canvas) for a weak camera or a counter scanner. The three confirmation screens (m-success, c-pay-success, w-success) play a two-note chime and vibrate through `chime()`, so both sides know the scan landed without watching the screen.
